/**
 * router-bootstrap (standard v1.27.3): progressive disclosure with zero pre-unlock — each stage sees only its own tools (anti-"haste" main line).
 *
 * 时序（用户定稿）：
 *   T0 首轮 = RL 句 + cwd/OS-home 锚定 + phase_begin（唯一确认工具，native；稳定 we）
...
const ROUTER_VERSION = 'v1.27.4'
/** glob/grep required string is `pattern`; bash/run_code own `description`. Gemini Flash mix-up. */
...
export function coerceSearchToolArgs(name, args) {
  if (!SEARCH_PATTERN_TOOLS.has(name)) return { args, coerced: false }
...
export function formatHelpParams(parameters, required) {
  const p = parameters && typeof parameters === 'object' && !Array.isArray(parameters) ? parameters : {}
  const schemaKeys = new Set(['type', 'properties', 'required', 'additionalProperties', 'description', 'title'])
  const props = p.properties && typeof p.properties === 'object' && Object.keys(p.properties).length
    ? p.properties
    : Object.fromEntries(Object.entries(p).filter(([k]) => !schemaKeys.has(k)))
  const req = Array.isArray(required) && required.length ? required : (Array.isArray(p.required) ? p.required : [])
  ...
  for (const k of keys) {
    const meta = props[k] || {}
    const need = req.includes(k) || meta.required === true
    lines.push('  - name=' + k + ' type=' + (meta.type || 'any') + (need ? ' required=true' : ' required=false'))
    if (meta.description) lines.push('    ' + String(meta.description))
  }
  return lines
}
export function enrichInvalidArgs(exec, result) {
  if (!result?.isError) return null
  const rawMsg = String(result.error?.message || '')
  const bodyText = Array.isArray(result.content) ? result.content.map((b) => b?.text || '').join('\n') : ''
  const msg = rawMsg || bodyText
  if (!/missing required property|invalid arguments/i.test(msg)) return null
  if (/received keys:/.test(msg) && /received keys:/.test(bodyText)) return null
  ...
  const text = 'Error: invalid arguments: ' + bits.join('; ')
  return { content: [{ type: 'text', text }], message: text }
}
/** Session cwd + OS home so the model expands `~` against reality instead of inventing /Users/<guess>. */
export function identityFacts(agent) {
  const home = homedir() || ''
  const cwd = agent?.session?.header?.cwd || ''
  return { home, cwd }
}
export function groundedPersona(agent) {
  const { home, cwd } = identityFacts(agent)
  let text = RL_PERSONA
  if (cwd) text += ' Working directory: ' + cwd + '.'
  if (home) text += ' OS home: ' + home + '.'
  if (home || cwd) text += ' Expand ~ to OS home; never invent a username.'
  return text
}
export function identityStageLine(agent) {
  const { home, cwd } = identityFacts(agent)
  if (!home && !cwd) return ''
  const parts = []
  if (cwd) parts.push('cwd=' + cwd)
  if (home) parts.push('OS home=' + home)
  return 'Paths: ' + parts.join('; ') + '. Expand ~ to OS home; never invent a username.'
}
...
const META_LIVE = [...META_TOOLS, 'dev_reload_preset_live', 'phase_begin', 'delivery_check', 'dev_reset_experience']
...
export function filterToolGuidance(sections, _stage, _fullNames) {
  // v1.27.3: do not drop tool:* docs as phases unlock — that rewrote the
  // middle of the system prompt (~22% stable prefix) and forced full cache
  // misses (铁律④). Restrict still hides non-callable tools from the schema.
  return sections || []
}
...
export function apply(ctx, config) {
  ...
  const invalidArgsStreak = new Map()
  ...
  ctx.on('tools/post-execute', async (exec, result, next) => {
    ...
    const key = sid + ':' + (exec?.name || '')
    const n = (invalidArgsStreak.get(key) || 0) + 1
    invalidArgsStreak.set(key, n)
    let text = enriched.message
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    const personaText = groundedPersona(agent)
    const planSection = (assembled.sections || []).find((s) => /plan/i.test(s.name))
    const baseSections = planSection
      ? [planSection, { name: 'router-persona', text: personaText, order: 0 }]
      : [{ name: 'router-persona', text: personaText, order: 0 }]

    if (!promoted) {
      // 首轮：无 restrict + 工具面只留 phase_begin（RL 句 + cwd/OS-home 锚定）
      // Keep runtime contexts (sandbox cwd, approval policy) — wiping them forced the
      // model to invent /Users/<guess> when expanding ~ (v1.27.2).
      return { ...assembled, sections: baseSections, tools: assembled.tools.filter((tool) => tool.name === 'phase_begin') }
    }

    // promoted：官方 sections 回流（persona = RL 句 + cwd/OS-home）+ 引导带宽控 + 阶段声明 + 常驻段
    ...
    const sections = filterToolGuidance((assembled.sections || []).map((s) =>
      /persona/i.test(s.name) ? { ...s, text: personaText } : s
    ), stage, fullNames)
    ...
    // v1.27.3: phase/tool-list is dynamic — park it at the tail (after first-party
    // tool docs 1000–2900 and SDK 5000) so the static prefix can cache (铁律④⑧).
    sections.push({ name: 'router-stage', order: 9100, text: stageText(stage, muteAwareList(runtimeCallable(toolsSvc, agent), memoryMuted(session)), memoryMuted(session), firstUserTask(session), identityStageLine(agent)) })
    // 声明与主动性常驻（人设常驻：不经压缩丢失；bootstrap 消息可能被 compaction 剪掉）
    sections.push({ name: 'router-decl', order: 9110, text: PROGRESSIVE_DECL })
    sections.push({ name: 'router-proactivity', order: 9120, text: PRESSURE_GUIDE.replace(/^\n+/, '') })
  ...
  registerTool({
    ...
    async execute(args) {
      ...
      if (doFresh) {
        ...
        const guide = START_GUIDE + '\n\n' + PROGRESSIVE_DECL + '\n\n' + PRESSURE_GUIDE + '\n\n' + stageText(0, runtimeList, memoryMuted(session), firstUserTask(session), identityStageLine(currentAgent()))
        try {
      ...
      const toolsSvc = currentAgent()?.ctx?.get?.('tools')
      const runtimeList = muteAwareList(runtimeCallable(toolsSvc, currentAgent()), memoryMuted(session))
      const guide = START_GUIDE + '\n\n' + PROGRESSIVE_DECL + '\n\n' + PRESSURE_GUIDE + '\n\n' + stageText(existing.stage, runtimeList, memoryMuted(session), firstUserTask(session), identityStageLine(currentAgent()))
      try {
        currentAgent()?.inbox.append('next-step', {
  ...
  registerTool({
    ...
    async execute(args) {
      ...
      const lines = ['工具: ' + name + ' [' + found.mark + ']', unlock, 'tool_description: ' + found.desc].filter(Boolean)
      lines.push(...formatHelpParams(params))
      return lines.join('\n')
    },
  })
  ...
  function installMetaShim(agent, opts) {
    ...
    n += make({
      ...
      execute: async (args) => {
        ...
        const lines = ['工具: ' + wanted + ' [' + mark + ']', unlock, 'tool_description: ' + (s.description || s.function?.description || '')].filter(Boolean)
        lines.push(...formatHelpParams(params))
        return lines.join('\n')
      },
    })
